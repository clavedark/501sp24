---
title: "Time issues in the OLS model"
author: "Dave Clark"
...
## Graphical Methods - state murder rate data
...
ggplot(states, aes(x=res, y=murder))+
  geom_point()+
  labs(y="murder rate" , x="residuals")+
    ggtitle("Murder Rate, Residuals")


ggplot(states, aes(x=res1, y=res))+
  geom_point()+
  geom_smooth(method=lm, se=FALSE)+
  labs(y="residuals" , x="residuals t-1")+
  ggtitle("Residuals, Residuals lagged one period")
...
Both panels suggest the residuals are not random across time.  The first panel plots the residuals against the $y$ variable. The second shows a relationship between the residuals and their lag ($t-1$), and it's pretty clearly not random.  So let's turn to some more mechanical ways of detecting autocorrelation.
...
The BG test is a more general test because it can account for higher order AR processes (while DW is limited to AR(1)).  Moreover, the BG test is simple to compute by hand and has few of the limitations listed above for DW. Note, this is not appropriate for panel data.
...
## Dynamic Models 
  ...
  - Lagging $y_t$ and including it on the right hand side of the regression equation changes the interpretation of the coefficients. This is one form of a dynamic model.

  - Lagging $y_t$ is powerful - it generally is a poor choice for dealing with autocorrelation. @keele2006dynamic [p. 203] show that, in presence of autocorrelation, lagging $y$ induces bias. @wooldridge2013 also shows that models with lagged $y$ variables violate the assumption $x$ and $e$ are uncorrelated - in other words, they induce endogeneity.

  - Lagging $x$ variables is also a great choice if theory implies the effect of $x$ is spread over time, and accumulates in some fashion - these models are known as *finite distributed lag models*. 


Dynamic models allow us to examine the long-term effects of changes in $x$ on $y_t$. One form of this is:
