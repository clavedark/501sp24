...
\noindent Recalling that the mean of a variable 
...
\noindent The same is true for the covariance assumption.
...
Note that these properties are true because we are minimizing the sum of the squared residuals. They do not have particular meaning otherwise regarding the errors, whether we meet assumptions of the model, etc. The next step, for next week, is to make a set of assumptions regarding the error term in order to facilitate statements about $\widehat{\beta}$, and inferences about those estimates. 
