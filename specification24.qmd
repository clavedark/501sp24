...
We're trying to explain variation. What does this look like?
  ...
  - best guess at variation in $y$ would be its mean (constant-only model). 
 with variables, variation is composed of two parts: $y = \mu + \epsilon$. The first is $x\beta$, our systematic component; the second is the random component. $x$ shapes the systematic part of $y$. 
...
<!-- In the early 1990s, there was a significant (and very boring) debate over these measures of model fit. The useful parts focused on what we, as a discipline, are trying to accomplish in modeling. There was no real resolution. \\~\\  -->
...
Permit nonlinear but monotonic[^*] changes in $y$ over changes in $x$. Model interpretations using natural logs of variables (from @wooldridge2013 p. 44)
